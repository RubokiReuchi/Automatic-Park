    ...
    void Update()
    {
        foreach (PerceptionEvent ev in events)
        {
            if (ev.sense == SENSE.VISION && ev.type == TYPE.SPOT)
            {
                ...
                if (ev.go_user.gameObject.CompareTag("Thief")) ev.go_user.SendMessage("Steal", ev.go_obj); // thief
                if (ev.go_user.gameObject.CompareTag("Policeman")) ev.go_user.SendMessage("Hide"); // thief
            }
