...
public class EventManager : MonoBehaviour
{
    ...
    void Update()
    {
        foreach (PerceptionEvent ev in events)
        {
            ...
            {
                if (ev.go_user.gameObject.GetComponent<BehaviorExecutor>().behavior.ToString() == "Thief") ev.go_user.SendMessage("Steal", ev.go_obj); // thief
                if (ev.go_user.gameObject.GetComponent<BehaviorExecutor>().behavior.ToString() == "Policeman") ev.go_obj.SendMessage("Hide"); // thief
